...
let new_userList = userList.map((user) => {
...
new_userList.forEach((user) => {
  console.log(user);
});

console.log('==============================');
//delete id === 1
new_userList = new_userList.filter((user) => user.id !== '1');
new_userList.forEach((user) => {
  console.log(user);
});
...
let updUser = new_userList.filter((user) => user.id === '2')[0];
console.log('updUser=', updUser);
updUser = { ...updUser, amount: 200 };
console.log('updUser=', updUser);
new_userList = new_userList.filter((user) => user.id !== '2');
new_userList = [...new_userList, updUser];
new_userList.forEach((user) => {
  console.log(user);
});
